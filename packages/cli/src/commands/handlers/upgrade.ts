  ...
  Effect.fn("cli.upgrade")(function* (input) {
    ...
    if (!method)
      return yield* Effect.fail(
        new Error("Could not detect the installation method. Pass --method to choose how to upgrade OpenClue."),
      )
    ...
    if (version === OPENCODE_VERSION) {
      log.warn(`OpenClue upgrade skipped: ${version} is already installed`)
      outro("Done")
      return
    }
