...
const envPath = process.env.OPENCLUE_BIN_PATH
...
const base = `openclue-${platform}-${arch}`
const binary = platform === "windows" ? "openclue.exe" : "openclue"
...
if (!resolved) {
  console.error(
    `It seems that your package manager failed to install the right ${command} binary package. Try manually installing ` +
