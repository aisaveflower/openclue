      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            ...
            <text fg={colors.muted}>openclue {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string) {
  ...
  const url = new URL("https://github.com/aisaveflower/openclue/issues/new?template=bug-report.yml")
  ...
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the openclue crash screen. If you can, describe what you were doing when it crashed.",
  )
  ...
  const marker = "\n... (truncated)"
  const head = `The openclue TUI crashed with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
