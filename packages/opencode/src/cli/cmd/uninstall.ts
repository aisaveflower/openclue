...
import { InstallationName } from "@opencode-ai/core/installation/version"
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: `npm uninstall -g ${InstallationName}`,
      pnpm: `pnpm uninstall -g ${InstallationName}`,
      bun: `bun remove -g ${InstallationName}`,
      yarn: `yarn global remove ${InstallationName}`,
      brew: `brew uninstall ${InstallationName}`,
      choco: `choco uninstall ${InstallationName}`,
      scoop: `scoop uninstall ${InstallationName}`,
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", InstallationName, "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
